...
def test_exact_transition_targets_all_fsm_transitions():
...
def test_transition_accuracy_is_one_with_nearest_register():
    _, fsm, state_codebook, input_codebook = _fixture()
    register = NearestAttractorRegister(state_codebook)
    transition = ExactCoordinateTransition(fsm, state_codebook, input_codebook)
    assert transition_accuracy(fsm, transition, register, state_codebook, input_codebook) == 1.0


def test_sparse_full_write_equals_exact_transition():
